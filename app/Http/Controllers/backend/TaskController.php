    ...
    public function TaskList(Request $request, $usersID)
{
    ...
    if ($user->role == 3) {
        ...
        $list = DB::table('createtask')->where('usersID', $usersID)->get();
        $employee = Employee::where('usersID', $usersID)->first();
        $usersID = $employee->usersID;
        $user = User::where('id', $usersID)->first();
        return view('backend.task.list_task', compact('list', 'user', 'employee'));
    }
}
...
public function TaskInsert(Request $request, $usersID)
{
    ...
    $data = [
        'usersID' => $user->id,
        'employeeName' => $user->name, // Use the id property of the Agent object
